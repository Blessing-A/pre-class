...
#To make the functions more consistent, one could perhaps rename them as normal_dist and multivariate_dist respectively
```
...
4. Use vapply() to:
    ...
    ```{r }
    df <- is.numeric (data.frame(x, y)) #created a data frame that is numeric
      for (i in colnames (df)) { #checked the data set for all the columns
        sd(df[[i]]) #found the standard deviation of the data frame for all the variables in the colnames
      }
    vapply(df, colnames, ...) #unable to apply the vapply
    ```
  
    b. Compute the standard deviation of every numeric column in a mixed data frame. (Hint: you’ll need to use vapply() twice.)
    
```{r }
#Unable to complete this question
```
